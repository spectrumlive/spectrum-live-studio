...
static void create_log_file(fstream &logFile)
{
	...
	get_last_log(false, "SPECTRUMLiveStudio/logs", lastLogFile);
#ifdef _WIN32
	get_last_log(true, "SPECTRUMLiveStudio/crashes", lastCrashLogFile);
#endif

	currentLogFile = GenerateTimeDateFilename("txt");
	dst << "SPECTRUMLiveStudio/logs/" << currentLogFile.c_str();
...
static int run_program(fstream &logFile, int argc, char *argv[])
{
	...
	try {
		...
		program.AppInit();
		delete_oldest_file(false, "SPECTRUMLiveStudio/profiler_data");
...
static void check_safe_mode_sentinel(void)
{
	...
	BPtr sentinelPath = GetAppConfigPathPtr("SPECTRUMLiveStudio/safe_mode");
