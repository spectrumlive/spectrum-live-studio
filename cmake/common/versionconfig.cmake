...
if(NOT DEFINED SPECTRUM_VERSION_OVERRIDE AND EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/.git")
  execute_process(
...
elseif(DEFINED SPECTRUM_VERSION_OVERRIDE)
  if(SPECTRUM_VERSION_OVERRIDE MATCHES "([0-9]+)\\.([0-9]+)\\.([0-9]+).*")
    string(
      REGEX REPLACE
      "([0-9]+)\\.([0-9]+)\\.([0-9]+).*"
      "\\1;\\2;\\3"
      _obs_version_canonical
      ${SPECTRUM_VERSION_OVERRIDE}
    )
    set(_obs_version ${SPECTRUM_VERSION_OVERRIDE})
  else()
...
list(GET _obs_version_canonical 0 SPECTRUM_VERSION_MAJOR)
list(GET _obs_version_canonical 1 SPECTRUM_VERSION_MINOR)
list(GET _obs_version_canonical 2 SPECTRUM_VERSION_PATCH)

set(OBS_RELEASE_CANDIDATE ${_obs_release_candidate})
set(OBS_BETA ${_obs_beta})

string(REPLACE ";" "." SPECTRUM_VERSION_CANONICAL "${_obs_version_canonical}")
string(REPLACE ";" "." SPECTRUM_VERSION "${_obs_version}")

if(OBS_RELEASE_CANDIDATE GREATER 0)
  message(
    AUTHOR_WARNING
    "******************************************************************************\n"
    "  + SPECTRUM-Studio - Release candidate detected, SPECTRUM_VERSION is now: ${SPECTRUM_VERSION}\n"
    "******************************************************************************"
  )
elseif(OBS_BETA GREATER 0)
  message(
    AUTHOR_WARNING
    "******************************************************************************\n"
    "  + SPECTRUM-Studio - Beta detected, SPECTRUM_VERSION is now: ${SPECTRUM_VERSION}\n"
    "******************************************************************************"
  )
endif()
